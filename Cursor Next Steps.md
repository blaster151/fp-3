...
- Pass 5 residual oracles: exported `RunnerOracles.residualMorphism` so the residual square checks surface through the standard oracle registry, bridged residual interaction-law summaries into functor/runner helpers, and exercised identity/mismatch/round-trip coverage in `test/residual-runner.spec.ts`.
