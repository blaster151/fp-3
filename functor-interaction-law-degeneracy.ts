...
import type { SetHom, SetObj, SetTerminalObject } from "./set-cat";
import type {
  ComonadCooperation,
...
const gatherBinaryArtifacts = <Obj, Arr, Left, Right, Value>(
  ...
  const kPrimeGap =
    "Provide coproduct diagonal factorisation k'_Y to complete Theorem 2 part (2).";
  ...
  steps.push(
    {
      ...
      description:
        "Factor through coproduct diagonals to obtain k'_Y as in Theorem 2 part (2).",
      object: component.object,
      metadata: artifacts.operationMetadata,
      gaps: [artifacts.kPrimeGap],
    },
    {
      label: "construct-kY",
      description:
  ...
  const traces = holds
    ? witnesses.flatMap((witness) =>
        witness.components.map<CommutativeBinaryDegeneracyTrace<Obj, Arr, Right>>((component) => {
          ...
          const artifacts = gatherBinaryArtifacts(witness, component, zeroComparison);
          return {
