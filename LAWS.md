...
- `makeFinitePullbackCalculator.induce(j, pullbackOfF, pullbackOfG)` now gathers every arrow that could mediate between the supplied cones and insists on a single survivor. The strengthened helper signals failure for both missing mediators and duplicate witnesses, so the returned arrow certifies uniqueness rather than relying on external checks. `test/pullback-induce.spec.ts` locks this in with regression coverage for the unique, absent, and duplicated cases.
