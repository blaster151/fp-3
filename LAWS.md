...
- **Statement**: `DiagramClosure.closeFiniteDiagram` realises Definition 82’s finite-diagram completion by adjoining identities for every object in the generated subcategory, iteratively inserting every ambient composite, and exposing the saturated `arrowLookup` so cone/cocone validators and limit witnesses can query newly synthesised morphisms.
- **Oracles**: `test/diagram-closure.spec.ts` seeds parallel generators inside a non-thin ambient category, checks that the closure materialises all identities and composites, and validates that the resulting functor assignment preserves domains, codomains, and composition across the saturated diagram.
