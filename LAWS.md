...
- **Statement**: `exponentialM2({ base, codomain })` enumerates the equivariant maps `B → C`, forms the induced idempotent on the function object, and equips the exponential with its evaluation morphism and currying helper so every arrow `A × B → C` factors uniquely, while `m2ExponentialComparison({ base, codomain, left, right })` produces the unique mediators between any two exponential witnesses and confirms they round-trip to the identities.
...
- **Oracles**: `test/m2-set.spec.ts` exercises the exponential builder by confirming the evaluation morphism is equivariant, demonstrating successful currying and reconstruction of legitimate mediators, detecting non-equivariant attempts, and verifying the uniqueness of the mediator. `test/laws/law.M2Exponential.spec.ts` layers on the comparison helper, checking that matching witnesses yield inverse mediators, mismatched data are rejected, non-equivariant mediators trigger failures, and evaluation factorizations remain observable.
- **Witness Builder**: `exponentialM2({ base, codomain })` returns the function object, its product with `B`, the evaluation arrow, and a currying helper that accepts any witnessed product `productM2({ left: A, right: B })` and arrow `A × B → C`.
