...
- **Statement**: `CategoryLimits.isProductForCone` and `CategoryLimits.isCoproductForCocone` now surface the synthesised mediators alongside their triangle and uniqueness verdicts, even when uniqueness fails, while `CategoryLimits.factorConeThroughProduct` and `CategoryLimits.factorCoconeThroughCoproduct` hand back the mediating arrows (or descriptive failure reasons) so cone morphisms in `Cone(D)` and `CoCone(D)` become executable witnesses for Definition 80 / Theorem 83 and expose the comparison arrows predicted by those statements.
...
- **Statement**: `CategoryLimits.arrowFromCone` / `CategoryLimits.coneFromArrow` (and their coproduct duals) now require a fixed limit or colimit witness in the sense of Theorem 84, so providing the canonical limit cone realises the bijection between cones over `D` with vertex `X` and arrows `X → L`. The helpers reject mismatched diagrams before consulting the universal property, propagate the constructed mediator from `isProductForCone`, and rebuild cones using the stored projections/injections so the two directions act as explicit inverses on the allowed data.
- **Oracles**: `test/mediators-generic.spec.ts` lifts Vect cones into mediating arrows, rebuilds the original legs from those arrows, confirms the round-trip in both directions, and now rejects malformed cones (bad legs or diagrams) as well as arrows aimed at the wrong product or coproduct objects.
