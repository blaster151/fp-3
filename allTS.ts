...
export const match = <T extends { _tag: string }>(t: T) => <R>(m: Matcher<T, R>): R => {
  // @ts-expect-error index by runtime tag
...
namespace TinyFpExamples {
  ...
  // --- Demo inputs (avoid 'name' to dodge the DOM global) ---
  type DemoUser = { name: string; email: string; age: number }
  const makeUser = (name: string, email: string, age: number): DemoUser => ({ name, email, age })

  // Your validators (ensure these exist as shown earlier)
  type VErrMsg = string
  const nonEmpty = validate<VErrMsg, string>('name empty', s => s.trim().length > 0)
  const emailLike = validate<VErrMsg, string>('email invalid', s => /@/.test(s))
  const adult     = validate<VErrMsg, number>('under 18', n => n >= 18)

  // Example inputs (rename to avoid the DOM 'name' global)
  const inputName  = 'Ada'
  const inputEmail = 'ada@example.com'
  const inputAge   = 17

  // If you defined concatArray and liftA3V like we discussed, this just works:
  const vUser =
    liftA3V<VErrMsg, string, string, number, DemoUser>(curry3(makeUser))(
      nonEmpty(inputName)
    )(
      emailLike(inputEmail)
    )(
      adult(inputAge)
    )

// vUser: Validation<VErrMsg, DemoUser>
  ...
  // Usage example
  const sumMonoid: Monoid<number> = { empty: 0, concat: (x, y) => x + y }
  concatAll(sumMonoid)([1,2,3,4]) // 10

  const totalNE = concatNE(sumMonoid)([1,2,3] as NonEmptyArray<number>) // 6




  // UE
  const fetchTask = fromPromise(() => fetch('https://api.example.com/slow'))
  const safeFetch = retry(3, 300)(withTimeout(2000)(fetchTask))

  const batched = allLimited(5)([/* 100+ tasks */])
  ...
  namespace DecoderExample {
    // Define a runtime schema for your type
    const UserD = object({
      id: stringD,
      email: refineD<string>('invalid email', s => /@/.test(s))(stringD),
      age: refineD<number>('must be >= 18', n => n >= 18)(numberD),
      role: optionalD(literalD('admin', 'user', 'guest')),
      tags: arrayOf(stringD),
      nickname: nullableD(stringD),
    })

    // Use it
    const good = decode(UserD)({
      id: 'u_123',
    ...
    const bad = decode(UserD)({
      id: 42,
      email: 'not-an-email',
      age: 12,
      tags: [1, 'ok'],
    })
    // bad: Err([...readable path messages...])

    // If you want Validation interop:
    const badV = toValidation(bad) // Validation<string, ...>
  }
}
...
namespace DecoderAsyncExample {
  // Runtime schema for a “User”
  const UserD = object({
    id: nonEmptyStringD,
    email: refineD<string>("invalid email", s => /@/.test(s))(stringD),
    age: refineD<number>("must be >= 18", n => n >= 18)(numberD),
    tags: withDefaultD(arrayOf(stringD), []),
  })

// Use the canonical env type exported with the helpers
type Env = EnvHttp

// Ensure your UserD returns a readonly array for tags:
//   tags: withDefaultD(arrayOf(stringD), [] as ReadonlyArray<string>)

const getUser = (
  id: string
): ReaderTaskDecoder<
  Env,
  { id: string; email: string; age: number; tags: ReadonlyArray<string> }
> => getJsonD(`/users/${id}`, UserD)

const getUserEmail: ReaderTaskDecoder<Env, string> =
  mapRTResult<
    Env,
    DecodeErr,
    { id: string; email: string; age: number; tags: ReadonlyArray<string> },
    string
  >((u) => u.email)(getUser("42"))


  // Example run (wrap in an async IIFE if top-level await is not allowed)
  ;(async () => {
    const env: Env = { apiBase: "https://api.example.com", http: (i, init) => fetch(i, init) }
    const r = await getUserEmail(env)
    if (isOk(r)) {
      console.log("email:", r.value)
    } else {
      console.error("decode failed:\n" + formatDecodeErrors(r.error))
    }
  })()
}
...
// =======================
// Examples (safe for TS Playground)
// =======================

namespace StateExamples {
  // simple counter/log demo using your State + sequenceState
...
namespace SRTExamples {
  ...
  // compose two pings using your SRT.combinators
  ...
  ;(async () => {
    ...
    // sFinal.calls: 2
    // (use console.log if you’re running this in a real environment)
  })()
}
...
namespace SRTNoHttpExample {
  type Env = { rand: () => number }
  type S = { attempts: number }

  // A step that randomly throws to simulate flakiness
  const flaky: StateReaderTask<Env, S, number> =
    (env) =>
    async (s0) => {
      const x = env.rand()
      if (x < 0.5) throw new Error('flaky step')
      const s1 = { ...s0, attempts: s0.attempts + 1 }
      return [x, s1] as const
    }

  // Make it robust with retry & timeout
  const robust = srtRetry(3, 50)(srtWithTimeout(500)(flaky))

  // Compose with a Result-carrying step
  const toResult: SRTResult<Env, S, string, number> =
    (env) => async (s) => {
      try {
        const [v, s1] = await robust(env)(s)
        return [Ok(v), s1] as const
      } catch (e) {
        return [Err(e instanceof Error ? e.message : String(e)), s] as const
      }
    }

  // Map over the Ok channel without touching S
  const squared = mapSRTResult<string, number, number>((n) => n * n)<Env, S>(toResult)

  // You can run this with:
  // (async () => {
  //   const [res, s1] = await runSRT(squared, { rand: Math.random }, { attempts: 0 })
  //   console.log(res, s1)
  // })()
}
...
namespace DoNotationExamples {
  ...
  // ----- TaskResult example (no DOM needed) -----
...
const tr: TaskResult<string, { total: number }> =
  DoTR<string>()
    .bind('x', delayOk(5, 2))
    .bind('y', delayOk(5, 3))
    .map(({ x, y }) => ({ total: x + y }))  // ← returns TaskResult
// no `.done()` here


  // (async () => console.log(await tr()))()
}
...
namespace PartitionSetExamples {
  ...
  void nums; void strs; void errs; void oks
}
...
// ----- RTO sample (pure, no DOM) -----
namespace RTO_Sample {
  type Env = { n: number }
  ...
  // run: await prog({ n: 2 })  // -> Some(5)
}

// ----- RWST sample -----
namespace RWST_Sample {
  type Env = { inc: number }
  type S = { log: ReadonlyArray<string> }
...
namespace OtherSampleX {
  export const walkJsonToEvents = (j: Json, emit: (e: JsonEvent) => void): void => {
  const go = (t: Json): void => {
    const n = t.un
    switch (n._tag) {
      case 'JNull': emit(ev.null()); break
      case 'JBool': emit(ev.bool(n.value)); break
      case 'JNum':  emit(ev.num(n.value)); break
      case 'JStr':  emit(ev.str(n.value)); break
      case 'JArr':
        emit(ev.startArr())
        for (const x of n.items) go(x)
        emit(ev.endArr())
        break
      case 'JObj':
        emit(ev.startObj())
...
// Option
const L2O = liftA2K1<'Option'>(OptionK)<number, number, number>(a => b => a + b)
//                             ^^^^^^^^^^ tell TS which F this is

const sumO = L2O(Some(2), Some(3)) // Some(5)

// ReaderTask<Env,_> with Env pinned
type Env = { base: number }
const RT = ReaderTaskK<Env>()
const add = (a: number) => (b: number) => a + b

const aRT: ReaderTask<Env, number> = ReaderTask.asks(e => e.base)
const bRT: ReaderTask<Env, number> = ReaderTask.map((n: number) => n * 2)(aRT)
