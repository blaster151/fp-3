export const FP_CATALOG = {
  ...
  finGrpRepresentationFunctorFromCatFunctor: 'Coerce a raw FinGrp→Vect CatFunctor into the enriched representation functor shape with law validation.',
  ...
  finGrpRepresentationNatTransFromNaturalTransformation: 'Upgrade generic FinGrp→Vect natural transformations into representation-aware form, revalidating the commuting matrix.',
  ...
  finGrpRepresentationHomSpace: 'Compute Hom-space bases, vector-space packages, coordinate/linear-map conversions, and additive/scale operations for FinGrp→Vect natural transformations.',
  makeFinGrpRepresentationEndomorphismAlgebra: 'Package Hom(ρ,ρ) for FinGrp representations with coordinate multiplication and structure constants.',
  ...
  collectFinGrpRepresentationSemisimplicitySummands: 'Flatten semisimplicity trees into direct-sum summands, verify leaf identities, and certify ambient reconstruction from ι∘π contributions.',
  collectFinGrpRepresentationIrreducibleSummands: 'Filter semisimplicity summands to those certified irreducible while surfacing reducibility witnesses and analysis failures.',
  ...
  runFinGrpRepresentationSemisimplicityWorkflow: 'Aggregate irreducibility, semisimplicity, summand, and certification diagnostics into a single workflow report with staged failures and success summaries.',
  formatFinGrpRepresentationSemisimplicityWorkflow: 'Render semisimplicity workflow results as a human-readable narrative with stage headlines, success/failure glyphs, and optional detailed transcripts.',
  summarizeFinGrpRepresentationSemisimplicityWorkflow: 'Condense semisimplicity workflow results into a classification, stage headlines, highlights, and actionable follow-up recommendations.',
  profileFinGrpRepresentationSemisimplicityWorkflow: 'Extract quantitative diagnostics from the semisimplicity workflow including tree depth, summand counts, irreducible verification, and certification failure modes.',
  formatFinGrpRepresentationSemisimplicityWorkflowProfile: 'Render semisimplicity workflow profiles as human-readable summaries with configurable sections covering representation metadata, workflow status, analysis metrics, summand dimensions, and certification outcomes.',
  reportFinGrpRepresentationSemisimplicityWorkflow: 'Bundle the semisimplicity workflow, summary, profile, and optional narrative into a single record for downstream reporting and analytics.',
  surveyFinGrpRepresentationSemisimplicityWorkflows: 'Aggregate multiple semisimplicity workflow reports, tally classifications and stage failures, and compute dimension statistics for representation analytics.',
  formatFinGrpRepresentationSemisimplicitySurvey: 'Render semisimplicity survey metrics into a human-readable summary with optional per-representation headlines, status checklists, recommendations, and narratives.',
