...
export interface SupervisedStackRoundTripResult<Obj, Arr, Left, Right, Value> {
  ...
  readonly coverageComparison?: SupervisedStackRoundTripCoverageComparison;
  readonly diagnostics?: ReadonlyArray<string>;
}
...
>(
  ...
  const baseResult: SupervisedStackRoundTripResult<Obj, Arr, Left, Right, Value> = {
    stack,
    reconstructed,
    mismatches,
  };
  const coverageComparison = compareSupervisedStackRoundTripCoverage(baseResult);
  const diagnostics: string[] = [
    `roundTrip: mismatches=${mismatches.length}`,
    ...mismatches.map((issue) => `roundTrip: ${issue}`),
  ];
  if (reconstructed.diagnostics.length > 0) {
    diagnostics.push(
      `roundTrip: reconstruction notes=${reconstructed.diagnostics.length}`,
      ...reconstructed.diagnostics.map((note) => `roundTrip: ${note}`),
    );
  }
  if (coverageComparison.issues.length > 0) {
    diagnostics.push(
      `roundTrip: coverage issues=${coverageComparison.issues.length}`,
      ...coverageComparison.issues.map((issue) => `roundTrip: coverage issue=${issue}`),
    );
  }
  if (coverageComparison.reconstructed) {
    const coverage = coverageComparison.reconstructed;
    diagnostics.push(
      `roundTrip: coverage interpreterExpected=${coverage.interpreterExpectedOperations}` +
        ` interpreterCovered=${coverage.interpreterCoveredOperations}` +
        ` kernelTotal=${coverage.kernelTotalClauses}` +
        ` kernelEvaluated=${coverage.kernelEvaluatedClauses}`,
    );
  }

  return { ...baseResult, coverageComparison, diagnostics };
};
