...
describe('Left adjunction (Σ): second triangle  ε ∘ (Σ_u η) = id on Σ_u A', () => {
  test('elementwise identity per fiber', () => {
    ...
    const eps = IndexedFamilies.counitSigmaEnum<J, I, { j: J; x: number }>(u, Jfin)      // ε_i
  ...
  test('Σ-side unit creates tagged pairs correctly', () => {
    ...
    const eta = IndexedFamilies.unitSigmaEnum<J, I, string>(u, Jfin)
  ...
  test('Σ-side counit extracts values correctly', () => {
    ...
    const eps = IndexedFamilies.counitSigmaEnum<J, I, string>(u, Jfin)
    
    // Test counit extraction
    const pair: { j: J; x: string } = { j: 1, x: 'extracted' }
    const result = eps(1 as I)(pair) // i doesn't matter for counit - just extracts x
  ...
  test('Σ-side second triangle round-trip', () => {
    ...
    const sigEta = IndexedFamilies.sigmaOfUnitEnum<J, I, number>(u, Jfin)
    const eps = IndexedFamilies.counitSigmaEnum<J, I, { j: J; x: number }>(u, Jfin)
    
    // Test round-trip for various elements
    const testElements: ReadonlyArray<{ j: J; x: number }> = [
      { j: 0, x: 42 },
      { j: 1, x: 17 },
      { j: 2, x: 99 },
      { j: 3, x: 123 }
