...
const compose = (g: Arrow, f: Arrow): Arrow => {
  ...
  if (f === arrowA && g === arrowC) return arrowC
...
describe('diagram functorization helpers', () => {
  const finiteShape = {
    ...
    id: idArrow,
    compose,
    src: (arrow: Arrow): Node => arrow.src,
    dst: (arrow: Arrow): Node => arrow.dst,
    eq: (left: Arrow, right: Arrow): boolean => left.name === right.name,
  }

  const functor = constructFunctorWithWitness(smallShape, baseCategory, {
  ...
  test('diagramToFunctorWitness reconstructs the identity functor', () => {
    ...
    const reconstruction = CategoryLimits.diagramToFunctorWitness({ base: baseCategory, diagram })

    expect(reconstruction.analysis.holds).toBe(true)
    expect(reconstruction.functor.report.identityCheck.holds).toBe(true)
    expect(reconstruction.functor.report.compositionCheck.holds).toBe(true)
  })
  ...
  test('diagramToFunctorWitness surfaces arrow endpoint failures', () => {
    ...
    const reconstruction = CategoryLimits.diagramToFunctorWitness({ base: baseCategory, diagram: brokenDiagram })
  ...
  test('analyzeCoconeNaturality validates dual coherence', () => {
    ...
    expect(failure.holds).toBe(false)
    expect(failure.legDiagnostics.some((diag) => !diag.holds)).toBe(true)
  })

  test('constantDiagram produces identity-on-object assignments', () => {
    const constant = CategoryLimits.constantDiagram({
      shape: smallShape,
      base: baseCategory,
      object: 'A',
    })

    expect(constant.onObjects('A')).toBe('A')
    expect(constant.onObjects('B')).toBe('A')
    expect(constant.onMorphisms(arrowB)).toEqual(identityA)
  })
})
